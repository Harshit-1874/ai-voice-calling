...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await fetch('https://ai-voice-calling-4u4w.onrender.com/call-logs');
  ...
  useEffect(() => {
    const fetchHubspotContacts = async () => {
      ...
      try {
        const res = await fetch('https://ai-voice-calling-4u4w.onrender.com/hubspot/contacts');
  ...
  const handleCall = async (e: React.FormEvent) => {
    ...
    setCallResult(null);
    try {
      const res = await fetch(`https://ai-voice-calling-4u4w.onrender.com/call/${encodeURIComponent(phoneNumber)}`, { 
