...
export default function CallCenter() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const res = await fetch('https://ai-voice-calling-8f7t.onrender.com/call-status');
  ...
  useEffect(() => {
    const fetchTemp = async () => {
      try {
        const res = await fetch('https://ai-voice-calling-8f7t.onrender.com/hubspot-temp');
        const data = await res.json();
  ...
  const handleCall = async () => {
    ...
    setCalling(true);
    try {
      await fetch(`https://ai-voice-calling-8f7t.onrender.com/call/${phoneNumber}`);
    } catch {}
    setCalling(false);
  };
