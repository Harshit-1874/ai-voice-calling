...
export default function CallCenter() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const res = await fetch('https://ai-voice-calling-4u4w.onrender.com/call-status');
  ...
  useEffect(() => {
    const fetchTemp = async () => {
      try {
        const res = await fetch('https://ai-voice-calling-4u4w.onrender.com/hubspot-temp');
        const data = await res.json();
  ...
  const handleCall = async () => {
    ...
    setCalling(true);
    try {
      await fetch(`https://ai-voice-calling-4u4w.onrender.com/call/${phoneNumber}`);
    } catch {}
    setCalling(false);
  };
