...
export default function Settings() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      setFetchError(null);
      try {
        const response = await fetch('https://ai-voice-calling-4u4w.onrender.com/constants');
        if (!response.ok) {
  ...
  const handleSaveSettings = async () => {
    ...
    setSaveMessage(null);
    try {
      const response = await fetch('https://ai-voice-calling-4u4w.onrender.com/constants/ai-configs', {
